...
import { getItems, deleteItem as deleteItemService, toggleItemAvailability, updateItem } from '@/services/itemService';
...
const toggleAvailability = async (item) => {
  try {
    ...
    await toggleItemAvailability(item._id, newAvailability);
    
    // Actualizar el ítem en la lista local
    const index = items.value.findIndex(i => i._id === item._id);
    if (index !== -1) {
      items.value[index].available = newAvailability;
    }
    
    success.value = `Ítem ${newAvailability ? 'disponible' : 'no disponible'}`;
    setTimeout(() => {
      success.value = '';
    }, 3000);
  } catch (err) {
    error.value = err.message || 'Error al cambiar la disponibilidad';
...
const handleEditSuccess = () => {
  showEditModal.value = false;
  success.value = 'Ítem actualizado correctamente';
...
const deleteItem = async () => {
  ...
  try {
    ...
    await deleteItemService(itemToDelete.value._id);
    ...
    success.value = 'Ítem eliminado correctamente';
    setTimeout(() => {
      success.value = '';
    }, 3000);
  } catch (err) {
...
// Obtener etiqueta para la condición
